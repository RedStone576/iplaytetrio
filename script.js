...
const barrier = Math.floor(Math.random() * (256) + 250)
const arr = ["a", "b", "c"]
...
function urmom(h)
{
    ...
    if (el.classList.contains("checkmark")) 
    {
        ...
        if (count === barrier) console.log(`★ WOW YOU DID IT ★\nTotal clicked: ${delCount}\nBarrier: ${barrier}`)
    }

    if (a.classList.contains("checkmark") && b.classList.contains("checkmark") && c.classList.contains("checkmark")) 
    {
        if (count >= barrier) return
        // me when when at the js user when new array
        arr.splice(arr.indexOf(lastToggled), 1)        
        toggle(document.getElementById(arr[Math.floor(Math.random() * 2)]))
        arr[0] = "a"
        arr[1] = "b"
        arr[2] = "c"
    }
